...
from .serializers import PostSerializer, UpvoteSerializer, CommentSerializers
...
class PostDetailAPIView(APIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        ...
        if serializer.is_valid():
            if post.user.id == request.user.id:
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
            return Response({'error': 'you are not authrized to edit this file'}, status=status.HTTP_401_UNAUTHORIZED)
    ...
    def delete(self, request, pk, *args, **kwargs):
        ...
        if post.user.id == request.user.id:
            post.delete()
            return Response({'res': 'object successfully deleted.'})
        return Response({'error': 'you are not authrized to edit this file'}, status=status.HTTP_401_UNAUTHORIZED)
...
class UpvoteAPIView(APIView):
    ...
    #         post.upvote_count -= 1
    #         post.upvote.filter(user = request.user).delete()
    #     else:
    #         post.upvote_count += 1
